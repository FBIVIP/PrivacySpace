...
class ConfigServer : XC_MethodHook() {
    ...
    fun start(classLoader: ClassLoader) {
        ...
        hookGetInstallerPackageNameMethod(classLoader)
    ...
    private fun hookGetInstallerPackageNameMethod(classLoader: ClassLoader) {
        ...
        if (!hooked) {
            XLog.e(
                RuntimeException("getInstallerPackageName not found"),
                "ConfigServer: config channel unavailable on this Android version/ROM."
            )
        }
    }
